...
export interface DeploymentPayload {
  ...
  /**
   * Whether post-deployment operations should be executed.
   * This is controlled from the Deployment UI and defaults to true.
   */
  runPostDeploymentOperations: boolean;
  metadata: {
    totalEntries: number;
    stepCount: number;
...
export function ConfigDataProvider({ children }: { children: React.ReactNode }) {
  ...
  const convertToDeploymentPayload = useCallback((): DeploymentPayload => {
    ...
    return {
      steps,
      postDeploymentOperations,
      runPostDeploymentOperations: true,
      metadata: {
