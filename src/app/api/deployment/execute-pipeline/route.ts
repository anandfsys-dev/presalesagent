  ...
  const stream = new ReadableStream({
    async start(controller) {
      ...
      try {
        ...
        for (let stepIndex = 0; stepIndex < payload.steps.length; stepIndex++) {
          ...
          for (let entryIndex = 0; entryIndex < step.entries.length; entryIndex++) {
            ...
            try {
              ...
              // Build a record-specific endpoint by injecting field values into any
        ...
        const postOps = payload.runPostDeploymentOperations ? payload.postDeploymentOperations || [] : [];
        if (postOps.length > 0) {
...
/**
 * Build a record-specific endpoint URL by replacing {FieldName} placeholders
 * in the configured endpoint template with values from the record.
 */
function buildEndpointForRecord(
  ...
  return template.replace(/\{([^}]+)\}/g, (fullMatch, key) => {
    ...
    if (value === undefined || value === null) {
      // Leave the placeholder intact if no value is available to avoid
      // accidentally producing an invalid URL segment.
      console.warn(`Endpoint template placeholder ${fullMatch} has no value in record`);
      return fullMatch;
    }
