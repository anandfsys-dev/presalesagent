...
export class SalesforceClient {
   ...
   *
   * If an explicit endpoint is provided, that endpoint will be used as-is.
   * Otherwise, the request will default to the standard sObject REST endpoint
   * based on the provided object API name.
   */
  async createRecord(
    objectType: string,
    ...
    try {
      ...
      // Normalise HTTP method: default to POST and strip any UI labels
      // like "POST (Create)" down to the verb for the actual request.
      const rawMethod = method || 'POST';
      const httpMethod = rawMethod.split(' ')[0].toUpperCase();
